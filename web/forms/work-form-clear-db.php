...
function Header()
{
    ...
    $this->Cell(80,10,'Resident Hours Submitted - June '.date('Y'),0,1,'C');
...
function FancyTable($header, $data)
{
    ...
    foreach($data as $key => $row1)
    {
		if( isset($row1['work_form']) ) {
			...
			$this->Ln(5);
			$this->SetFont('','B');
			$this->Cell(30,10,$row1['name'],1,0,'C');
			// Line break
			$this->Ln(10);
			...
			foreach( $row1['work_form'] as $row ){
				...
				$this->Cell($w[1],6, date('d/m/Y', strtotime($row['date']) ),'LR',0,'C',$fill) ;
...
//$pdf->Output(dirname(__FILE__). '../files/archive/work-credit-submissions/all-resident-details-'.date('Y-m-d').'-'.time().'.pdf', 'F');
// $pdf->Output();
...
foreach( $data as $key => $row_single ) {
	...
	if(isset($row_single['work_form'])) {
		...
		//$pdf->Output(dirname(__FILE__). '/pdf-files/single-resident-detail-'.$row_single['name'].'-'.date('Y-m-d').'-'.time().'.pdf', 'F');
...
echo "<script>alert('you have cleared the work submitted db, you may now add hours for the next month..');document.location='../'</script>";
